...
- **v0.3.7**（2026-09-19）：**📐 双屏对齐体检（方案 D）** —— 用户实测反馈「两块屏都用 4K 时，跨屏铺满后画面左右对不齐、主屏底部还缺一块」。**根因（先算清楚再动手）**：一个窗口跨两块屏时 Windows 只用**一个像素比例**、不做逐屏缩放，所以「跨屏能对得齐」的唯一条件是**两屏竖向像素高度 ≈ 与各自物理高度成正比**（等价于竖向像素密度 PPI 一致）。本机实测：主屏 27" 3840×2160 → 竖向 **161 像素/英寸**；副屏 16" 竖放 2400×3840 → 竖向 **283 像素/英寸**；两者竖向像素 2160 vs 3840 → 虚拟屏高取 3840，**较矮的主屏底部会被切掉 43.8%**，且 PPI 差 **75%** 导致同一条横线在两屏物理高度不同。而两屏可用分辨率的竖向档位 `{2160,1440,1080}` 与 `{3840,2560,1600}` **交集为空** → 现有模式怎么选都对不上，这就是「4K 就不齐」的原因（低清晰度时凑巧接近所以能铺满）。**本轮只做只读分析（用户指定不动系统设置）**：① 新增 C# `DisplayModes(dev)`（`EnumDisplaySettingsW` + 完整 `DEVMODEW` 结构）枚举每屏**全部可用分辨率**；② 新增 `edid_sizes()` 从注册表读 EDID 的物理尺寸（cm）；EDID 未上报的屏（本机副屏 `MTI1B1A` 就是）支持**手动填对角线英寸**（`set_screen_inches`，存进基线文件，按原生宽高比推算物理宽高）；③ 新增 `align_report()`：逐屏给出物理尺寸来源、画布高、竖向 PPI、原生比例、可用分辨率数量；诊断出「谁会被切掉百分之几」「PPI 差多少」（并明确说明 **Windows 的 DPI 缩放 100%/150% 不影响这件事**，只有分辨率组合能改）；在共同高度上搜索候选组合并按「高度误差 ×3 + 宽高比偏离原生 ×12 + PPI 差」排序，标注是否变形/是否对齐良好；再给出**理想方案**（保留高分辨率时，哪一屏需要 `高度 × 原生比例` 的**自定义分辨率**，并指出在 NVIDIA 控制面板 / Intel 显卡命令中心里加，建议只添加不应用、设错 15 秒自动回滚）。界面新增「📐 双屏对齐体检」卡片（体检 / 复制结论 / 保存屏幕尺寸），全流程只读、不改任何显示设置。
