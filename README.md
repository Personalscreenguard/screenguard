...
- **v0.3.6**（2026-09-19）：**一键检测 + 一键修复 + 三处「自动修复打架」修复**。① **新增「🩺 一键检测」**：点一下把**所有与屏幕有关的状态**扫一遍并逐项判定（正常/警告/问题）+ 给处置建议 —— 显示器（分辨率/刷新率/主副屏/DDC/亮度/音量/色彩配置/横竖方向）、显示链路近 24 小时掉线次数、HDR 支持与当前状态（含厂商 `AsHDRControl` 是否接管）、总亮度(gamma)是否在原曲线、音频端点列表（标注可调/固定音量/系统默认）、小米屏 MiTV 接口可达性与音量、电池电量与健康度、恢复基线是否完整。结果**写日志**：`%APPDATA%\Screenguard\logs\diag-<时间>.log` 每次一个文件，并追加一行总览到 `diag-history.log`；界面有「📂 打开日志文件夹」。**开机时会自动跑一次并落日志**，偶发掉线事后可翻日志对比。② **新增「🔧 一键修复」**（用户要求）：只修软件层能确定的事 —— 有掉线记录/枚举不到屏→强制重协商链路；总亮度偏离 100%→复位；当前控制的是固定音量端点→自动切到可调端点；基线不完整→补齐；有窗口被铺满→还原；随后**自动复查**并给出前后对比。硬件/系统限制类（DDC 链路本身、厂商 HDR 服务、固定音量端点）不硬修，只如实说明。③ **修「自动修复打架」**（根因同 v0.3.5 的待机回黑）：切旋转/写缩放/铺满/HDR/色彩都会产生显示事件，自动修复线程当成「掉线」去强制重协商，**把用户有意改的状态又改回去**（实测：点「副屏方向」当场生效、几秒后自己变回竖屏）。新增 `mark_change()` / `recently_changed(secs)` 闸门：任何主动改动显示配置的操作都会打时间戳，自动修复在 120 秒内不插手。④ 界面温感优化：「窗口拖到另一屏不变小」/「还原缩放」点完明确提示**必须注销或重新登录才生效**（Windows 限制：改每屏缩放只能写注册表，无运行时 API）。⑤ **HDR / 色彩的实测结论（不再给漂亮但可能不实的报告）**：HDR 侧用独立 P/Invoke 把 `SetDisplayConfig` 的 **8 种标志组合全试了一遍，全部返回 87（ERROR_INVALID_PARAMETER）**，且 `hdr_states` 读回值在切换前后始终不变 → 本机 HDR 由厂商服务 `AsHDRControl` 接管，公开 API 改不动；因此把按钮文案从「2 台已切换（注意…）」改成**先说没生效**并给出改用路径，界面另加一条常驻说明。色彩侧：`apply_color_space` 实测成功返回，看不出变化是因为目标与当前一致（两屏本就都是 sRGB），已写进说明。
- **v0.3.5**（2026-09-19）：**待机防回黑 · 小米音量目标式追赶 · 恢复默认覆盖全部软件改动**。① **修「待机后鼠标唤醒、屏又立刻黑下去」**：根因不是待机本身，而是 v0.3.1 的自动修复 —— 屏幕待机会让系统产生「显示器掉线」事件，自动修复线程看到就去修，而**深度修复内部含有 `screen_off()`**，于是刚被鼠标唤醒的屏又被关回去。修法：`screen_off()` 打上「是我们主动关的」标记 + `standby_state()` 在检测到用户输入时自动判定已唤醒并清标记 + **待机期间自动修复一律不执行**。② **小米音量改为「目标式追赶」**：原实现点一次就朝那个目标一路走完，导致「拖回 20%」得先把 30% 走完才肯降；现改为前端每次拖动只更新**目标值**（`AtomicI32`），后端循环每步重读最新目标决定方向 —— 拖回立刻反向。实测：起始 15 → 目标 30 → 0.8s 后改目标 20 → t+1.2s=18 → t+2.4s=**20 停住** → 还原 15。③ **步进间隔放宽**：原 70ms 高频连打把显示器 MiTV 服务打到不应答（ping 通、6095 超时，约 20 分钟后自恢复），改为 **220ms/步 + 失败等 2s 重试 + 回读失败等 1.2s 重试**。④ **音频端点枚举补全**：`EnumAudioEndpoints` 的 mask 从 `1`(ACTIVE) 改为 `15`(STATEMASK_ALL)，连未激活/未插入端点一起列出（端点 2 → **10 个**，副屏音频可见）；注意对未激活端点做 `Activate` 会抛 `0x80070003`，必须逐个 try 包住。⑤ **恢复默认覆盖全部软件改动**：新增 缩放对齐 / 各屏旋转方向 / 小米音量 / 音频控制设备 四项还原（原本只有 总亮度/铺满/电源/HDR/ICC/音量），并给基线加「补齐」逻辑（老装机基线缺字段时自动补，不必清库）。⑥ **掉线提醒贴右下角**：定位漏了显示器**原点偏移**（只拿 `size()` 硬算「屏宽-窗宽」），多屏布局下算到别处 → 改为 `pos.x + size.width - 窗口宽 - 10`，实测 `x=2190,y=1278`（工作区 2560×1392，右/下各留 10px）。
- **v0.3.4**（2026-09-19）：**小米音量打通 · 状态化按钮**。① **小米屏音量真正可控了**（新增关键能力）：小米这块屏不支持 DDC/CI（换过 Type-C 口、Intel/NVIDIA 两条分支都实测不通），它的 HDMI/DP 音频端点在 Windows 里又是**固定音量**，两条常规路都堵死；但实测发现显示器本体（Android/hyperOS）的 MiTV Assistant **能读能写音量**：`getvolume` 读、`keyevent&keycode=volumeup/volumedown` 每次 ±1（实测 10→11→10），没有 `setvolume`（404）→ 因此用「读-步进-回读」逼近目标值实现了 `mitv_volume_set`，界面在「系统控制」里新增 **小米音量** 滑块（接口不可达时自动隐藏），并把「固定音量端点」的提示改为引导用户用这个滑块。② **HDR 按钮改为状态显示**：不再让用户"选"，直接显示当前状态（`HDR：开` / `HDR：关` / `HDR：混合（1/2 开）` / `本机不支持`），点一下才把所有屏设成一致；面板的 HDR 格子同样显示状态。③ **副屏方向两个按钮合并成一个**：原「横竖屏切换 + 恢复副屏竖屏」合并为 `副屏方向：竖屏(2400x3840)（点击切换）`，按钮直接显示当前方向，点一下切换（注意：后端返回的是 `resolution` 字符串而非 w/h 字段，解析要用字符串）。④ 亮度同步改为**以主屏亮度为基准**并把"读不到独立亮度的屏由总亮度控制"如实写进结果。踩坑：把「读-步进-回读」整段塞进一个 PowerShell 脚本反而因引号/转义失败（报 `ERR:connect`），改回 Rust 循环调用已验证可用的单步函数。
- **v0.3.3**（2026-09-19）：**安全阀版本 —— 选窗铺满 · 一键恢复默认 · 退出自动还原 · 音量端点可选**。起因是用户实测踩到一个真问题：**用「铺满」把自己的 ScreenGuard 窗口铺满后，窗口底部被裁掉，界面上「还原」按钮点不到，屏幕直接卡住**。本轮针对性修复 + 补齐安全阀：① **铺满改成「先选窗口」**：新增 `ListWindows`（枚举可见、有标题、尺寸正常的顶层窗口）与 `SpanWindow(hwnd)`，界面上给出下拉列表让用户挑目标窗口，不再只能对"当前前台"下手；② **绝不铺满自己的窗口**：`SpanForeground/SpanWindow` 增加归属校验（关键：**PID 必须由 Rust 侧传入** —— 这段 C# 跑在 powershell.exe 里，`GetCurrentProcessId()` 拿到的是 PowerShell 的 PID，用它排除"自己"是错的，实测窗口列表里真的出现了 ScreenGuard 自己）；③ **三个不依赖界面的还原入口**：全局快捷键 **`Ctrl+Alt+R`** 还原被铺满的窗口、托盘右键新增「还原铺满的窗口」「恢复默认状态」、面板快捷控制新增「还原窗口」「恢复默认」格子（铺满后就算界面被盖住也一定能还原）；④ **一键恢复默认 + 退出自动还原**（用户新要求）：应用启动 7 秒后采集一次**基线快照**（`%APPDATA%\Screenguard\state.json`：HDR 各屏状态、ICC 各屏配置、音量当时值；总亮度视作 100% 原始曲线），「恢复默认」把**所有软件层面改动**（总亮度 / HDR / 色彩配置 / 被铺满的窗口 / 屏幕电源 / 音量）一次性回到基线，**退出软件时默认自动执行**（可关）；基线只采一次不覆盖，避免第二轮启动把"改过的状态"当原始值；⑤ **音量滑块可用了**：默认输出端点若是**固定音量**的 HDMI/DP 显示器音频（Windows 自己的滑块也调不动，实测本机就是这种），旧版只会把滑块禁用；现在新增 `AudioList/AudioSetDev/AudioMuteDev` 枚举**所有活动输出端点**并给出下拉选择，选中哪个就控制哪个，固定音量的会明确标注「（固定音量·不可调）」并提示换设备；⑥ **HDR 老实汇报**：`SET_ADVANCED_COLOR_STATE` 之后补 `SetDisplayConfig(QDC_DATABASE_CURRENT)` 再 Apply，并把**逐台原始明细**一并回传给界面 —— 实测本机 API 返回成功但读回未变（HDR 很可能被厂商/驱动接管），因此不再给"已切换"这种漂亮但可能不实的结论；⑦ 「铺满播放器 / 还原播放器」两个按钮**移除**：它们依赖 `player_hwnd()` 查找播放器进程，实测「还原播放器」会卡住不返回（invoke 超时），且与通用铺满重复；⑧ 修掉「铺满」连点两次会覆盖备份导致还原失效的 bug（同窗口已铺满时保留旧备份）。新增踩坑：**PowerShell 脚本文件若含中文且无 BOM，PS 5.1 按 ANSI 读会直接语法报错**（本轮又踩一次，改成纯 ASCII 脚本）；`HWND` 是 64 位，JSON 迷你解析器必须返回 long 否则截断后 `IsWindow()` 失败。
